...
const WaterUsageGame = () => {
  ...
  useEffect(() => {
    const fetchAndSetMonthlyAverage = async () => {
      try {
        const response = await fetch('https://data.calgary.ca/resource/j7mp-h975.json?$order=date ASC');
      ...
      objects.forEach(object => {
        this.physics.add.overlap(player, object, () => {
          if (lastTriggeredObject !== object) {
            ...
            setWaterUsage(prev => prev + (waterData[object.type] || 0));
            ...
            if (waterUsage + (waterData[object.type] || 0) > dailyLimit) endGame.call(this, 'fail');
            else if (clickCount + 1 === 10 && waterUsage <= dailyLimit) endGame.call(this, 'success');
          }
        });
      });
  ...
  const handleSaveAndQuit = async () => {
    ...
    if (token && window.phaserScene) {
      ...
      await fetch('http://localhost:5000/api/game/save', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token },
        body: JSON.stringify(gameData),S
      });
  ...
  return (
    <div className="water-game-container">
      ...
      <Scoreboard
        ...
        score={Math.round(dailyLimit - waterUsage)}
      />
      ...
      {!isPaused && (
        <div className="button-overlay" style={{ position: 'absolute', top: '780px', left: '55%', transform: 'translateX(-50%)', display: 'flex', flexDirection: 'column', gap: '10px', alignItems: 'center' }}>
          <button className="submit-btn" onClick={() => { playClickSound(); setIsPaused(true); if (window.phaserScene) window.phaserScene.scene.pause(); }}>⏸ Pause</button>
          <button className="submit-btn" onClick={() => { playClickSound(); navigate('/menu'); }}>Main Menu</button>
        </div>
      )}
      {isPaused && (
